...
const inputClass = 'block w-full rounded-lg border border-stone-300 crm-surface bg-white p-2 text-sm';
export default function PartnerPaymentsView({ kind = 'vendor', currentUser, onSelectCustomer }) {
    ...
    const filterDateField = kind === 'vendor' ? 'dispatch_date' : config.date;
    ...
    const filtered = useMemo(() => rows.filter(row =>
        ...
        matchesPaymentMonth(row[filterDateField], month) &&
    ...
    ), [rows, partner, status, search, config, kind, month, filterDateField]);
    ...
    return <section className="space-y-4">
        <div className="crm-page-header flex justify-between gap-3"><h2 className="text-lg font-bold">{config.title}</h2><button className="rounded-lg border px-3 py-2 text-sm" onClick={load} disabled={loading}>Refresh</button></div>
        ...
        <div className="flex flex-wrap items-end gap-3"><label className="text-sm">{kind === 'vendor' ? 'Dispatch month' : 'Payment month'}<input type="month" className={inputClass} value={month} onChange={e=>setMonth(e.target.value)}/></label><button className="rounded-lg border px-3 py-2 text-sm" onClick={()=>setMonth('')}>All months</button></div>
        <p className="text-xs text-stone-500">Combined amounts follow the selected {config.label.toLowerCase()}, month, status and search. {kind === 'vendor' ? 'Month uses dispatch date for all payment statuses. Projects without a dispatch date appear under All months.' : 'Month uses the payment date; unpaid or undated records appear under All months.'}</p>
        {loading ? <p>Loading payment records…</p> : error ? <div role="alert" className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800"><p>Payment records could not be loaded: {error}</p><p className="mt-2">If the new payment columns are missing, run add_manual_partner_payments.sql in Supabase, then refresh.</p></div> : <>
            <div className="rounded-2xl border border-stone-200 bg-white border-l-4 border-l-brand-500 p-5"><p className="text-sm font-semibold">Combined amount · {partner || `All ${config.label.toLowerCase()}s`} · {filtered.length} projects</p><p className="mt-1 text-xl font-bold">{formatPayment(combined.value)}</p>{combined.missing>0&&<p className="text-xs text-amber-800">{combined.missing} missing amounts excluded</p>}</div>
            <div className="grid gap-3 sm:grid-cols-3">{[['Paid',formatPayment(totals.paid/100)],['Unpaid',formatPayment(totals.unpaid/100)],['Not recorded',totals.unrecorded]].map(([label,value])=><div key={label} className={`rounded-2xl border p-4 ${label === 'Paid' ? 'border-stone-200 bg-white border-t-2 border-t-brand-500' : label === 'Unpaid' ? 'border-stone-200 bg-white border-t-2 border-t-sunshine-500' : 'border-stone-200 bg-white border-t-2 border-t-stone-300'}`}><p className="text-xs font-semibold text-stone-600">{label} · current filters</p><p className="mt-1 text-lg font-bold">{value}</p></div>)}</div>
            <div className="overflow-x-auto rounded-xl border crm-surface bg-white"><table className="w-full text-left text-sm"><thead className="crm-table-head bg-amber-50 text-amber-900"><tr>{['Customer',config.label,'Amount','Status',...(kind === 'vendor' ? ['Dispatch date'] : []),'Payment date','Actions'].map((label,index)=><th key={index} className="p-3">{label}</th>)}</tr></thead><tbody>{filtered.map(row=><tr key={row.id} className="border-t"><td className="p-3"><button className="text-left font-semibold underline" onClick={()=>onSelectCustomer?.(row)}>{row.consumer_name || row.consumer_number}</button><p className="text-xs text-stone-500">{row.consumer_number}</p></td><td className="p-3">{row[config.name] || '—'}</td><td className="whitespace-nowrap p-3">{formatPayment(row[config.amount])}</td><td className="p-3">{row[config.status] || 'Not recorded'}</td>{kind === 'vendor' && <td className="p-3">{row.dispatch_date || 'Not recorded'}</td>}<td className="p-3">{row[config.date] || '—'}</td><td className="p-3">{canEdit && <button className="rounded-lg border px-3 py-1" onClick={()=>openEditor(row)}>Edit payment</button>}</td></tr>)}{!filtered.length && <tr><td colSpan={kind === 'vendor' ? 7 : 6} className="p-5 text-stone-500">No matching customers.</td></tr>}</tbody></table></div>
        </>}
        {editing && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4"><form role="dialog" aria-modal="true" aria-labelledby="partner-payment-title" onSubmit={save} className="w-full max-w-md space-y-4 rounded-2xl crm-surface bg-white p-6"><h3 id="partner-payment-title" className="font-bold">{config.label} payment · {editing.consumer_name}</h3>
