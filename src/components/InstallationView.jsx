...
export default function InstallationView({ onSelectCustomer, isChannelPartnerOffice, partnerName, channelPartnerFilter }) {
    ...
    const fetchCustomers = useCallback(async (pageNum = 0, isAppend = false) => {
        ...
        try {
            ...
            console.error('Error fetching installation customers:', err);
            if (!isAppend) setCustomers([]);
            setHasMore(false);
        } finally {
            setLoading(false);
        ...
        <div className="space-y-6 animate-in fade-in duration-300">
            ...
            <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-between">
                ...
                <div className="flex items-center gap-2 self-end sm:self-center">
                    ...
                    <span className="text-xs font-bold text-stone-500 bg-stone-100 px-3 py-2 rounded-xl">
                        Total: <span className="text-stone-900 font-extrabold">{totalCount.toLocaleString('en-IN')}</span> Installations
                    </span>
                </div>
            </div>
            ...
            {loading ? (
                ...
                <div className="flex flex-col items-center justify-center h-64 bg-white rounded-3xl border border-dashed border-stone-200 p-8 text-stone-400 text-center">
                    <Wrench className="w-10 h-10 mb-3 text-stone-300" />
                    <p className="font-bold text-stone-600 text-sm">No installation records found</p>
