...
import { inventoryStatus } from '../utils/inventoryStatus';
...
export default function GodownInventoryView() {
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            for (let start = 0; ; start += 500) {
                const { data, error } = await supabase.from('godown_inventory').select('*').or('is_sample.is.null,is_sample.eq.false').order('sr_no').order('id').range(start,start+499);
    ...
    return <section className="space-y-4">
        <div className="crm-page-header flex flex-wrap items-center justify-between gap-3"><h2 className="text-lg font-bold">Godown Stock</h2><div className="flex gap-3"><button className="rounded-lg border px-3 py-2 text-sm" onClick={()=>edit(null)}>Add item</button><button className="rounded-lg border px-3 py-2 text-sm" onClick={load} disabled={loading}>Refresh</button><button className="rounded-lg bg-stone-900 px-3 py-2 text-sm text-white" onClick={()=>setShowBom(true)}>Create BOM</button></div></div>
        <div className="flex gap-3"><button className={`rounded-lg border px-4 py-2 ${tab==='stock'?'crm-primary-button bg-stone-900 text-white':''}`} onClick={()=>setTab('stock')}>Stock</button><button className={`rounded-lg border px-4 py-2 ${tab==='bom'?'crm-primary-button bg-stone-900 text-white':''}`} onClick={()=>setTab('bom')}>View / Download BOMs</button><button className={`rounded-lg border px-4 py-2 ${tab==='history'?'crm-primary-button bg-stone-900 text-white':''}`} onClick={()=>setTab('history')}>Stock history</button></div>
        {tab==='bom'?<SavedDocumentsView refreshKey={refreshKey}/>:tab==='history'?<GodownStockHistory refreshKey={refreshKey}/>:<>
        <p className="text-sm text-stone-500">Sufficient means above the reorder level. Low stock means at or below it; BOM availability is checked against the quantities required.</p>
