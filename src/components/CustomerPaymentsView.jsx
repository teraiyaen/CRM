...
import { paymentLeft, formatPayment, totalPayments, matchesPaymentMonth } from '../utils/customerPayments';
...
export default function CustomerPaymentsView({ onSelectCustomer }) {
    ...
    const [dateField, setDateField] = useState('payment_date');
    ...
    const rows = customers.filter(customer => {
        ...
        return matchesDealer && matchesPaymentMonth(customer[dateField], month) && `${customer.consumer_name || ''} ${customer.consumer_number || ''} ${dealer}`.toLowerCase().includes(search.toLowerCase());
    });
    const totals = [
    ...
    return <section className="space-y-4">
        <div className="flex items-center justify-between"><h2 className="text-lg font-bold">Customer Payments</h2><button disabled={loading} onClick={load} className="rounded-lg border px-3 py-2 text-sm">Refresh</button></div>
        ...
        <label className="block text-sm font-semibold">Dealer
            <select value={dealerFilter} onChange={e=>setDealerFilter(e.target.value)} className="mt-1 block w-full rounded-xl border bg-white p-3 text-sm sm:max-w-xs">
        ...
        <p className="text-xs text-stone-500">Totals combine the customers matching your filters. Received amounts are cumulative, not receipts collected during the selected month. Undated customers are excluded when a month is selected.</p>
        {!loading && !error && <div className="grid gap-3 sm:grid-cols-3">{totals.map(([label,total])=><div key={label} className="rounded-xl border bg-white p-4"><p className="text-xs text-stone-500">{label} · {rows.length} customers</p><p className="mt-1 text-lg font-bold">{formatPayment(total.value)}</p>{total.missing>0&&<p className="text-xs text-amber-700">{total.missing} missing amounts excluded</p>}</div>)}</div>}
        {error && <p role="alert" className="text-red-700">Payments could not be loaded: {error}</p>}
        {loading ? <p>Loading payments…</p> : !error && <div className="overflow-x-auto rounded-xl border bg-white"><table className="w-full text-left text-sm"><thead className="bg-stone-100"><tr>{['Customer','Consumer number','Dealer','Quotation amount','Amount received','Payment date','Left'].map(label=><th key={label} className="p-3">{label}</th>)}</tr></thead><tbody>{rows.map(customer => {
            const left = paymentLeft(customer.actual_payment, customer.payment);
            return <tr key={customer.id} className="border-t"><td className="p-3"><button className="text-left font-semibold underline" onClick={()=>onSelectCustomer(customer)}>{customer.consumer_name || 'Unnamed customer'}</button></td><td className="p-3">{customer.consumer_number}</td><td className="p-3">{customer.dealer || '—'}</td><td className="whitespace-nowrap p-3">{formatPayment(customer.actual_payment)}</td><td className="whitespace-nowrap p-3">{formatPayment(customer.payment)}</td><td className="whitespace-nowrap p-3">{customer.payment_date || '—'}</td><td className={`whitespace-nowrap p-3 font-bold ${left !== null && left < 0 ? 'text-red-700' : ''}`}>{formatPayment(left)}</td></tr>;
